...
```
```{r}
students %>% group_by(Race) %>% summarise(new_mean=round(sum(Avg.score)/n(),2)) %>% ggplot(aes(Race,new_mean,fill=Race))+geom_bar(stat='identity')+geom_text(aes(label = new_mean)) + ylab("Mean Test Score")
```
